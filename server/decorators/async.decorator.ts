...
export function MeasureAsyncTime(target: any, methodName: string, descriptor: PropertyDescriptor) {
  ...
  descriptor.value = async function (...args: any[]) {
    ...
    try {
        ...
        get: (target: any, prop: string | symbol) => {
          ...
          if (value && typeof value.then === 'function') {
            ...
            return value.then((result: any) => {
              ...
              Logger.info(
                `Method ${methodName} await #${currentAwaitCount} at ${awaitLocations.get(currentAwaitCount)} took ${awaitDuration}ms`,
              )
              return result
            })
          }
      ...
      Logger.info(`Method ${methodName} completed in ${duration}ms with ${awaitCount} await operations`)
      return result
    } catch (error) {
      const endTime = Date.now()
      const duration = endTime - startTime
