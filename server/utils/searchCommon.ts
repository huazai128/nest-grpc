import { DateQueryDTO, KeywordDTO, SiteIdQueryDTO, KeyIdQueryDTO } from '@app/models/query.model'
 ...
 */
export class SearchDTO extends IntersectionType(KeywordDTO, DateQueryDTO, KeyIdQueryDTO) {
  @IsString()
  @IsNotEmpty()
  @IsOptional()
  path?: string
 ...
 */
export class SearchCommonDTO extends IntersectionType(SearchDTO, SiteIdQueryDTO) {}
 ...
 */
export function handleCommonSearchKeys<T extends SearchDTO>(query: T, KEYS: Array<string>): FilterQuery<T> {
 ...
 * @return {*}  {FilterQuery<T>}
 */
export function handleSearchKeys<T extends SearchCommonDTO>(query: T, KEYS: Array<string>): FilterQuery<T> {
 ...
 */
export function iterateQueryParams<T>(queryParams: Record<string, any>, pKey?: string): FilterQuery<T>['$and'] {
  let processedParams: FilterQuery<T>['$and'] = []
  for (const [key, value] of Object.entries(queryParams)) {
    const nKey = pKey ? `${pKey}.${key}` : key
    if (isObject(value)) {
      const subParams: FilterQuery<T>['$and'] = iterateQueryParams<T>(value, nKey)
...
export function projectHourOption(params?: object): PipelineStage.Project {
  return {
    $project: {
      day: { $dateToString: { date: '$create_at', format: '%Y-%m-%d', timezone: 'GMT' } },
 ...
 * @return {*}  {PipelineStage.Group}
 */
export function groupHourOptionId(params: object, isDay: boolean, idParmas = {}): PipelineStage.Group {
  if (!isDay) {
    idParmas = {
      hour: '$_id.hour',
      ...idParmas,
