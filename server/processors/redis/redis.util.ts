...
export function retryStrategy(retries: number): number | null {
  const errorMessage = ['[Redis]', `retryStrategy！retries: ${JSON.stringify(retries)}`]
...
export function createRedisConnection(options: RedisModuleOptions) {
  ...
  switch (type) {
    ...
    case 'single':
      ...
      const connectionOptions: RedisOptions = { retryStrategy: retryStrategy, ...commonOptions, port, host }
